...
**Input**: User description: "Build a Content Brain and Strategist Agent for the Marlon Avery portfolio — an ongoing, gitignored knowledge store that ingests websites, YouTube videos, podcast transcripts, resume, and local docs, normalizes them into structured entries, and a Claude-powered Strategist (website content expert for thought leaders) that reads new entries and proposes site improvements to maximize Marlon Avery's brand. Propose-only; every real site change flows through the spec-driven workflow. No fabrication; strict employer confidentiality."
...
**Independent Test**: Add a website URL, a YouTube URL, a podcast transcript file, a resume, and a general local document. Confirm each becomes a normalized entry with a summary, tags, key facts, and the correct `docKind`; confirm the raw source is preserved; confirm re-adding the same source is skipped; confirm nothing landed in version control.
...
3. **Given** a local file (markdown, text, PDF, or Word document), **When** Marlon adds it, **Then** its text is extracted and normalized into an entry with `docKind: general`.
4. **Given** a podcast transcript file, **When** Marlon adds it, **Then** it is ingested as `docKind: podcast-transcript`; the normalizer extracts speaker insights, episode themes, and any Marlon-attributed statements as `about-marlon` key facts.
5. **Given** Marlon's resume file (PDF or docx), **When** he adds it, **Then** it is ingested as `docKind: resume`; the normalizer extracts roles, skills, achievements, and quantified outcomes as `about-marlon` key facts available for site use after verification.
6. **Given** a source that was already added, **When** Marlon adds the same source again, **Then** the system detects the duplicate and does not create a second entry.
7. **Given** any ingested source, **When** Marlon inspects the repository's version control status, **Then** none of the Brain's contents (entries, raw sources, index, proposals) appear as tracked or committable files.
8. **Given** a source containing employer-sensitive material, **When** it is normalized, **Then** the resulting entry is flagged as potentially confidential.
9. **Given** facts extracted from a source, **When** the entry is created, **Then** facts asserting things about Marlon are distinguished from general/background knowledge so they can be verified before any site use.

---

### User Story 2 — The Strategist Proposes Site Improvements (Priority: P2)

After new material is in the Brain, Marlon asks the Strategist to review it. The Strategist is a website content expert specializing in thought leadership — its primary goal is to maximize Marlon Avery's brand. It reads the new entries alongside the current state of the site and produces concrete, written proposals: blog post ideas with outlines, copy updates, new project/case-study candidates, and specific SEO/GEO improvements. It reasons about how each piece of Brain material can be turned into content that builds Marlon's authority, discoverability, and positioning as a platform-engineering thought leader. Each proposal is grounded in real Brain entries and never invents facts. The Strategist writes proposals to a review folder; it never edits the live site.
...
- **FR-003**: The system MUST support local documents in markdown, plain text, PDF, and Word (.docx) formats, including podcast transcripts (text/markdown) and resume files (PDF/docx). Each local document MUST be classified with a `docKind` (`podcast-transcript`, `resume`, or `general`) so the normalizer and Strategist can apply appropriate reasoning to each type.
