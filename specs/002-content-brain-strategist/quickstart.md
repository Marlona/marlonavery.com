...
npm run brain:add -- ./path/to/notes.md            # docKind: general (auto-detected)
npm run brain:add -- ./path/to/podcast-transcript.txt   # docKind: podcast-transcript (filename heuristic or --kind flag)
npm run brain:add -- ./path/to/Marlon_Avery_Resume.pdf  # docKind: resume (filename heuristic)
# Also test: npm run brain:add -- ./path/to/resume.docx -- --kind resume
```

**Expected**: each command prints a new entry id + title; `brain/entries/<id>.md` exists with a
populated summary, tags, and keyFacts; podcast-transcript and resume entries show `docKind` in
frontmatter and have `about-marlon` keyFacts; `brain/sources/<id>.<ext>` holds the raw capture;
