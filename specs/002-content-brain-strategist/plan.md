...
YouTube transcripts, podcast transcripts, resume, and local docs (md/txt/pdf/docx), normalizes
each into a structured entry via Claude (with `docKind`-aware extraction for podcast transcripts
and resume), and tracks them in a JSON manifest. Add a **Strategist** (a Claude Code subagent +
`/brain-review` skill) that acts as a website content expert for thought leaders, with the
primary goal of maximizing Marlon Avery's brand — it reads unprocessed entries plus current site
state and writes propose-only recommendations (blog/copy/project/SEO/GEO) to `brain/proposals/`.
...
│       └── doc.ts               # local md/txt/pdf/docx → text; detects docKind (podcast-transcript/resume/general)
└── prompts/
