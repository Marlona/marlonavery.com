...
| `docKind` | `'podcast-transcript' \| 'resume' \| 'general'?` | Only set when `sourceType === 'doc'`. Drives normalizer context. |
...
| `docKind` | `'podcast-transcript'\|'resume'\|'general'` | required when `sourceType === 'doc'`; omitted for web/youtube |
...
The normalizer prompt receives `docKind` as context when `sourceType === 'doc'` so it applies appropriate extraction logic:
- `podcast-transcript`: extract speaker insights, episode themes, Marlon-attributed quotes/positions as `about-marlon` facts
- `resume`: extract roles, skills, achievements, quantified outcomes as `about-marlon` facts
- `general`: standard extraction
